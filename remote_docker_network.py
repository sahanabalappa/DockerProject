...
''')
		...
		if i==1:
			...
			os.system(f"ssh {remote_main_menu.remoteIP} docker network create --driver {inp1} --subnet {inp2} {inp}")
		elif i==3:
			inp=input("NETWORK NAME : ")
			inp1=input("CONTAINER NAME : ")
			os.system(f"ssh {remote_main_menu.remoteIP} docker network disconnect {inp} {inp1}")
		elif i==4:
			inp=input("CONTAINER NAME : ")
			inp1=input("NETWORK NAME : ")
			inp2=input("IMAGE NAME : ")
			os.system(f"ssh {remote_main_menu.remoteIP} docker run -it --name {inp} --network {inp1} {inp2}")
		elif i==5:
			inp=input("NETWORK NAME : ")
			os.system(f"ssh {remote_main_menu.remoteIP} docker network inspect {inp}")
		elif i==6:
			inp=input("NETWORK NAME : ")
			os.system(f"ssh {remote_main_menu.remoteIP} docker network rm {inp}")
		elif i==0:
